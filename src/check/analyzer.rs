...
impl Analyzer {
    pub(super) fn new(rules: &[SelectedRule]) -> Self {
        ...
        for rule in rules {
            let item = metadata
                ...
                .unwrap_or_else(|| {
                    panic!("required analyzer rule {} is not registered", rule.code)
                });
    ...
    pub(super) fn run(&self, source: &str, spec: &SpecFile<Span>) -> Vec<Finding> {
        LintSession::from_config(&self.config)
            .run(spec, source)
            .into_iter()
